...
# export DAT_PATH='/home/xray/dat'
DAT_PATH=${DAT_PATH:-/home/xray/dat}

# You can set this variable whatever you want in shell session right before running this script by issuing:
# export JSON_PATH='/home/xray/config'
JSON_PATH=${JSON_PATH:-/home/xray/config}

# Set this variable only if you are starting xray with multiple configuration files:
# export JSONS_PATH='/home/xray/config'

# Set this variable only if you want this script to check all the systemd unit file:
# export check_all_service_files='yes'

# Gobal verbals
mkdir -p /home/xray/{log,dat}

if [[ -f '/etc/systemd/system/xray.service' ]] && [[ -f '/home/xray/xray' ]]; then
...
get_current_version() {
  ...
  if [[ -f '/home/xray/xray' ]]; then
    CURRENT_VERSION="$(/home/xray/xray -version | awk 'NR==1 {print $2}')"
...
install_file() {
  NAME="$1"
  if [[ "$NAME" == 'xray' ]]; then
    install -m 755 "${TMP_DIRECTORY}/$NAME" "/home/xray/$NAME"
...
install_xray() {
  # Install Xray binary to /home/xray/ and $DAT_PATH
  install_file xray
  ...
  if [[ "$NO_LOGFILES" -eq '0' ]]; then
    if [[ ! -d '/home/xray/log/' ]]; then
      install -d -m 755 -o 0 -g 0 /home/xray/log/
      install -m 600 -o "$INSTALL_USER_UID" -g "$INSTALL_USER_GID" /dev/null /home/xray/log/access.log
      install -m 600 -o "$INSTALL_USER_UID" -g "$INSTALL_USER_GID" /dev/null /home/xray/log/error.log
      LOG='1'
    else
      chown 0:0 /home/xray/log/
      chmod 755 /home/xray/log/
      chown "$INSTALL_USER_UID:$INSTALL_USER_GID" /home/xray/log/*.log
      chmod 600 /home/xray/log/*.log
    fi
  fi
}
...
[Service]
User=$INSTALL_USER
Environment=XRAY_LOCATION_ASSET=/home/xray/dat
${temp_CapabilityBoundingSet}
${temp_AmbientCapabilities}
${temp_NoNewPrivileges}
ExecStart=/home/xray/xray run -config /home/xray/config/config.json
Restart=on-failure
...
${temp_AmbientCapabilities}
${temp_NoNewPrivileges}
ExecStart=/home/xray/xray run -config /home/xray/config/%i.json
Restart=on-failure
RestartPreventExitStatus=23
LimitNPROC=10000
LimitNOFILE=1000000
...
[Service]
ExecStart=
ExecStart=/home/xray/xray run -confdir $JSONS_PATH" |
...
[Service]
ExecStart=
ExecStart=/home/xray/xray run -config ${JSON_PATH}/config.json" > \
      '/etc/systemd/system/xray.service.d/10-donot_touch_single_conf.conf'
    echo "# In case you have a good reason to do so, duplicate this file in the same directory and make your customizes there.
# Or all changes you made will be lost!  # Refer: https://www.freedesktop.org/software/systemd/man/systemd.unit.html
[Service]
ExecStart=
ExecStart=/home/xray/xray run -config ${JSON_PATH}/%i.json" > \
      '/etc/systemd/system/xray@.service.d/10-donot_touch_single_conf.conf'
  fi
...
remove_xray() {
  if systemctl list-unit-files | grep -qw 'xray'; then
    ...
    local delete_files=('/home/xray/xray' '/etc/systemd/system/xray.service' '/etc/systemd/system/xray@.service' '/etc/systemd/system/xray.service.d' '/etc/systemd/system/xray@.service.d')
    ...
    if ! ("rm" -r "${delete_files[@]}"); then
      ...
      if [[ "$PURGE" -eq '0' ]]; then
        ...
        if [[ -n "$JSONS_PATH" ]]; then
          echo "info: e.g., $JSONS_PATH and /home/xray/log/ ..."
        else
          echo "info: e.g., $JSON_PATH and /home/xray/log/ ..."
        fi
      fi
      exit 0
    fi
...
main() {
  ...
  if [[ "$LOG" -eq '1' ]]; then
    echo 'installed: /home/xray/log/'
    echo 'installed: /home/xray/log/access.log'
    echo 'installed: /home/xray/log/error.log'
  fi
